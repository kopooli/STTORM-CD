...
DEVICE = "cpu"
BASE_LOG_DIR = "attachments/test_metrics_logs"
os.makedirs(BASE_LOG_DIR, exist_ok=True)


def load_model(checkpoint_path, size, initialized_metrics, dataset, method=None, dataset_name=None, index=None, cos_baseline=False):
    latent_dim = 128
...
if __name__ == "__main__":
    ...
    for folder_name, ravaen_flag, disaster_type, index in disasters:
        ...
        # Load models with explicit names and folder_name as dataset_name
        models = {
            "small_original": load_model(
                os.path.join(original_path, "small_modified_checkpoint.ckpt"),
                "small", initialized_metrics, test_dataset,
                method="small_original", dataset_name=folder_name
            ),
            "medium_original": load_model(
                os.path.join(original_path, "medium_modified_checkpoint.ckpt"),
                "medium", initialized_metrics, test_dataset,
                method="medium_original", dataset_name=folder_name
            ),
            "large_original": load_model(
                os.path.join(original_path, "large_modified_checkpoint.ckpt"),
                "large", initialized_metrics, test_dataset,
                method="large_original", dataset_name=folder_name
            ),

            "small_my_variable": load_model(
                os.path.join(my_checkpoint_path, "small_variable.ckpt"),
                "small", initialized_metrics, test_dataset,
                method="small_my_variable", dataset_name=folder_name
            ),
            "medium_my_variable": load_model(
                os.path.join(my_checkpoint_path, "medium_variable.ckpt"),
                "medium", initialized_metrics, test_dataset,
                method="medium_my_variable", dataset_name=folder_name
            ),
            "large_my_variable": load_model(
                os.path.join(my_checkpoint_path, "large_variable.ckpt"),
                "large", initialized_metrics, test_dataset,
                method="large_my_variable", dataset_name=folder_name
            ),

            "small_my_fixed": load_model(
                os.path.join(my_checkpoint_path, "small_fixed.ckpt"),
                "small", initialized_metrics, test_dataset,
                method="small_my_fixed", dataset_name=folder_name
            ),
            "medium_my_fixed": load_model(
                os.path.join(my_checkpoint_path, "medium_fixed.ckpt"),
                "medium", initialized_metrics, test_dataset,
                method="medium_my_fixed", dataset_name=folder_name
            ),
            "large_my_fixed": load_model(
                os.path.join(my_checkpoint_path, "large_fixed.ckpt"),
                "large", initialized_metrics, test_dataset,
                method="large_my_fixed", dataset_name=folder_name
            ),
            ...
            "Index": load_model(
                os.path.join(my_checkpoint_path, "small_fixed.ckpt"),
                "small", initialized_metrics, test_dataset,
                method="Index", dataset_name=folder_name, index=index, cos_baseline=False
            ),
            "Cos_baseline": load_model(
                os.path.join(my_checkpoint_path, "small_fixed.ckpt"),
                "small", initialized_metrics, test_dataset,
                method="Cos_baseline", dataset_name=folder_name, index=None, cos_baseline=True
            ),
        }
